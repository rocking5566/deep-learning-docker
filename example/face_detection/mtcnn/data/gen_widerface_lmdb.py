...
# https://drive.google.com/file/d/1tKRX90H_az1oiM6YewxWbHS7n3mpgal8/view?usp=sharing
anno_file = r"/datasets/wider_face/wider_face_split/wider_face_anno.txt"
im_dir = r"/datasets/wider_face/WIDER_train/images"


def gen_widerface_lmdb(image_size=48):
    # save_dir = "{0:d}".format(image_size)
    # pos_save_dir = "{0:d}/positive".format(image_size)
    # part_save_dir = "{0:d}/part".format(image_size)
    # neg_save_dir = '{0:d}/negative'.format(image_size)
    # os.mkdir(save_dir)
    # os.mkdir(pos_save_dir)
    # os.mkdir(part_save_dir)
    # os.mkdir(neg_save_dir)

    f1 = open('pos_{0:d}.txt'.format(image_size), 'w')
    f2 = open('neg_{0:d}.txt'.format(image_size), 'w')
    f3 = open('part_{0:d}.txt'.format(image_size), 'w')
    ...
    for annotation in annotations:
        ...
        while neg_num < 10:
            ...
            if np.max(Iou) < 0.15:  # output class label only
                # Iou with all gts must below 0.15. Original implementation is 0.3
                f2.write(r"xx\negative\%s" % n_idx + ' 0\n')
                im = resized_im
                h, w, ch = resized_im.shape
                if h != image_size or w != image_size:
                ...
                # save_file = os.path.join(neg_save_dir, "%s.jpg" % d_idx)
                # success = cv2.imwrite(save_file, resized_im)
                # if not success:
                #     raise Exception('Not writing file!')

    # limit the amount of part  and negative sample to keep
    ...
    fid = open("{0:d}_roi.lmdb".format(image_size), 'wb')
    pickle.dump(roi_list, fid)
    fid.close()

    fid = open("{0:d}_cls.lmdb".format(image_size), 'wb')
...
if __name__ == '__main__':
    gen_widerface_lmdb(12)
    gen_widerface_lmdb(24)
    gen_widerface_lmdb(48)
