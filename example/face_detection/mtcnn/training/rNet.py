...
with open(r'../data/24_cls.lmdb', 'rb') as fid:
    cls = pickle.load(fid)
with open(r'../data/24_roi.lmdb', 'rb') as fid:
    roi = pickle.load(fid)
...
img_input = Input(shape=[24, 24, 3])  # change this shape to [None,None,3] to enable arbitraty shape input
x = Conv2D(16, (3, 3), strides=1, padding='same', name='conv1')(img_input)
c1out = PReLU(shared_axes=[1, 2], name='prelu1')(x)
c1out = concatenate([c1out, img_input], axis=3)
...
for i_train in range(80):
    # still need to run manually on each batch to tune the loss
    randx = random.choice([0, 1, 1])
    batch_size = 64
    print ('currently in training macro cycle: ', i_train, randx)
    if i_train == 0:
    ...
    if 0 == randx:
        model = Model([img_input], [classifier])
    ...
    if 1 == randx:
        model = Model([img_input], [bbox_regress])
...
def save_model(m):
    m.get_layer('bbox1').set_weights(bbox_weight)
    m.get_layer('classifier1').set_weights(cls_weight)
    m.save_weights('model24.h5')
